...
function load_from_hash() {
	...
	if (!isNaN(index)) open_file(index);
	else location.hash = "0";
}

document.getElementById("prev").onclick = () => {
	let index = parseInt(location.hash.slice(1), 10) || 0;
	if (index > 0) location.hash = `${index - 1}`;
};

document.getElementById("next").onclick = () => {
	let index = parseInt(location.hash.slice(1), 10) || 0;
	if (index < JsxFiles.length - 1) location.hash = `${index + 1}`;
};

document.addEventListener("keydown", (event) => {
	const index = parseInt(location.hash.slice(1), 10) || 0;
	if (event.key === "ArrowLeft" && index > 0) location.hash = `${index - 1}`;
	else if (event.key === "ArrowRight" && index < JsxFiles.length - 1) location.hash = `${index + 1}`;
